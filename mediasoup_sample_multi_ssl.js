...
const webServer = https.createServer(options, app).listen(webPort, function () {
  console.log(
    "Web server start. https://serverurl:" + webServer.address().port + "/"
  );
});
...
wsServer.on("connection", function connection(ws) {
  ...
  ws.on("close", function () {
    console.log(
      "client closed. id=" + getId(ws) + "  , total clients=" + getClientCount()
    );
    cleanUpPeer(ws);
  });
...
function preparePeer(ws, message, downOnly) {
  ...
  peerconnection.on("signalingstatechange", function () {
    console.log(
      "-- PeerConnection.signalingstatechanged, state=" +
...
function handleAnswer(ws, message) {
  ...
  peerconnection
    ...
    .then(function () {
      ...
      dumpPeer(
        peerconnection.peer,
        "peer.dump after setRemoteDescription(answer):"
      );
    })
    .catch((err) => {
...
function dumpPeer(peer, caption) {
  ...
  console.log(
    caption + " transports=%d receivers=%d senders=%d",
    peer.transports.length,
    peer.rtpReceivers.length,
    peer.rtpSenders.length
  );
}
